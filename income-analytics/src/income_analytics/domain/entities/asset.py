...
class Asset(Entity):
    ...
    def __post_init__(self) -> None:
        ...
        if len(name) > 200:
            raise ValueError(
                "Asset name cannot have more than 200 characters."
            )
        ...
        if self.isin is not None:
            ...
            if not isinstance(self.isin, str):
                raise ValueError(
                    "Asset ISIN must be a non-empty string when provided."
                )
        ...
        if self.created_at.tzinfo is None:
            raise ValueError(
                "Asset timestamps must be timezone-aware."
            )
        ...
        if self.updated_at < self.created_at:
            raise ValueError(
                "Asset updated_at cannot be before created_at."
            )
